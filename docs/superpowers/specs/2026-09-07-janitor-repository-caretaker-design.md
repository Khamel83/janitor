...
* **Semantic Input Hashing:** Computes SHA-256 over `(git diff stat, external commit SHAs, status)`. If input hash matches the last recorded run in `~/.local/state/janitor/state.json`, exits without calling the model.
* **Sentinel-Preserved Reconciliation:**
  1. Summarizes verified commits and diff stats (`HEAD~1..HEAD`).
  2. Updates `CONTEXT.md` strictly within `<!-- janitor:begin:focus -->` and `<!-- janitor:begin:recent -->` sentinels.
  3. Updates `TODO.md` (checking off items only with verified commit evidence or explicit human checks).
  4. Stages only `CONTEXT.md` and `TODO.md`, verifies `git diff --cached --name-only`, and commits:
     ```bash
